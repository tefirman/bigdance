...
def mock_bracket_with_teams():
    ...
    for region in ["East", "West", "South", "Midwest"]:
        for seed in range(1, 17):
            teams.append(Team(f"Team{seed}_{region}", seed, region, 1800 - (seed * 30), "Conference"))
...
class TestESPNScraper:
    def test_initialization(self, sample_cache_dir):
        ...
        assert scraper.base_url == "https://fantasy.espn.com/games/tournament-challenge-bracket-2025"
        ...
        assert women_scraper.base_url == "https://fantasy.espn.com/games/tournament-challenge-bracket-women-2025"
            ...
            with patch("webdriver_manager.chrome.ChromeDriverManager"):
                with patch("selenium.webdriver.chrome.options.Options"):
                    ...
                    with patch.object(ESPNScraper, "_retrieve_page") as mock_retrieve:
                        mock_retrieve.return_value = {1: SAMPLE_POOL_HTML, 2: "<html>Page 2</html>"}
    ...
    def test_get_bracket(self, sample_cache_dir):
        ...
        with patch.object(ESPNScraper, "get_page") as mock_get_page:
            ...
            assert "https://fantasy.espn.com/games/tournament-challenge-bracket-2025/bracket?id=" in call_args
...
class TestESPNBracket:
    ...
    def test_extract_bracket(self, sample_cache_dir):
        ...
        with patch("bigdance.wn_cbb_scraper.Standings") as mock_standings:
            ...
            with patch.object(bracket_handler, "extract_bracket", 
                            wraps=bracket_handler.extract_bracket) as mock_method:
...
class TestGameImportanceAnalyzer:
    ...
    def test_print_importance_summary(self):
        ...
        game_importance = [
            {
                ...
                "all_entries_impact": [
                    {"name": "Entry1", "win_pct_team1": 0.7, "win_pct_team2": 0.45, "win_pct_baseline": 0.6, "impact": 0.25},
                    {"name": "Entry2", "win_pct_team1": 0.3, "win_pct_team2": 0.4, "win_pct_baseline": 0.35, "impact": 0.1}
                ]
            }
        ]
        ...
        with patch("builtins.print") as mock_print:
            ...
            mock_print.assert_any_call("Warning: Entry 'NonExistentEntry' not found in the analysis data.")
