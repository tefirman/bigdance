...
class BaseScraper:
    ...
    def _get_cached_response(self, cache_key: str, max_age: int = DEFAULT_CACHE_EXPIRY) -> Optional[str]:
        """
...
class ESPNScraper(BaseScraper):
    ...
    def get_page(self, url: str, cache_key: Optional[str] = None, 
                check_pagination: bool = False, max_pages: int = 10) -> Optional[Union[str, Dict[int, str]]]:
        """
        ...
        if not check_pagination and cache_key:
            cache_expiry = DEFAULT_CACHE_EXPIRY if "blank" in cache_key else DEFAULT_ENTRY_CACHE_EXPIRY
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    logger.warning(f"Retrieval attempt {attempt+1} failed: {e}. Retrying in {wait_time}s...")
    ...
    def _retrieve_page(self, url: str, cache_key: Optional[str], 
                      check_pagination: bool, max_pages: int) -> Union[str, Dict[int, str]]:
        """
                ...
                if pagination:
                    ...
                    while page_num < max_pages:
                        ...
                        if not next_button or self._is_button_disabled(next_button):
                            logger.debug(f"No more pages or next button disabled (at page {page_num})")
                            break
...
class ESPNBracket:
    ...
    def __init__(self, women: bool = False, cache_dir: Optional[str] = None):
        ...
        try:
            ...
            logger.info(f"Successfully loaded {len(self.ratings_source.elo)} teams from Warren Nolan")
        except Exception as e:
...
class GameImportanceAnalyzer:
    ...
    def analyze_win_importance(self, current_round: Optional[str] = None, 
                              num_sims: int = 1000) -> List[Dict]:
        """
        ...
        for game_ind in range(len(teams_in_round)//2):
            ...
            if team1 in actual_bracket.results[current_round] \
            ...
            logger.debug(f"Matchup impact: {game_analysis['max_impact']:.4f} (max), "
    ...
    def _analyze_matchup(self, team1: Team, team2: Team, round_name: str, 
                        actual_bracket, baseline, num_sims: int) -> Dict:
        """
        ...
        results_team1 = self.pool.simulate_pool(num_sims=num_sims, fixed_winners=fixed_winners_team1)
        ...
        results_team2 = self.pool.simulate_pool(num_sims=num_sims, fixed_winners=fixed_winners_team2)
    ...
    def print_importance_summary(self, game_importance: List[Dict], entry_name: Optional[str] = None) -> None:
        """
...
def main():
    ...
    if options.as_of:
        ...
        if options.as_of not in round_names:
            ...
            if options.as_of in ["First", "Second", "Sweet", "Elite", "Final"]:
                logger.warning('Hot tip: make sure to put multi-word round names in quotes, '
    ...
    pool_results = pool_sim.simulate_pool(num_sims=1000, fixed_winners=pool_sim.actual_results.results)
